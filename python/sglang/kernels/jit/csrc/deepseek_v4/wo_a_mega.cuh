...
__global__ void __cluster_dims__(Trait::kSplitK, 1, 1) __launch_bounds__(Trait::kThreads, 2) wo_a_mega_kernel(
    ...
    const int32_t* __restrict__ positions,  // [T]
...
#pragma unroll
    for (int i = 0; i < kTokPerThread; ++i) {
      ...
      if (t < m) rot[i] = reinterpret_cast<const float2*>(freqs + positions[t] * kRopeDim)[pair];
    }
#pragma unroll
    for (int h = 0; h < kHeadsPerCta; ++h) {
      const int s = (h + 1) * kHeadStages - 1;
 ...
 * \param positions [T] int32 absolute token positions.
...
inline void wo_a_mega_run(
  ...
  using namespace wo_a_mega;  // make_map, wo_a_mega_kernel
  ...
  CHECK_HOST(q.has_value() != y.has_value()) << "wo_a_mega: pass either (q, scales) or y, not both and not neither";
  CHECK_HOST(q.has_value() == scales.has_value()) << "wo_a_mega: q and scales come as a pair";
  if (q.has_value()) {
  ...
  CHECK_HOST(m >= 1 && m <= kMMax) << "wo_a_mega supports 1 <= T <= " << kMMax << ", got " << m;
  ...
  [[maybe_unused]] static const auto _ = [] {
    CHECK_CUDA(
        cudaFuncSetAttribute(wo_a_mega_kernel<Trait>, cudaFuncAttributeMaxDynamicSharedMemorySize, sizeof(Smem)));
    CHECK_CUDA(cudaFuncSetAttribute(wo_a_mega_kernel<Trait>, cudaFuncAttributePreferredSharedMemoryCarveout, 100));
    return 0;
  }();
  LaunchKernel(kClusters * kSplitK, kThreads, device, sizeof(Smem))
      .config({.use_pdl = true, .cluster_dim = dim3{kSplitK, 1, 1}})(
          wo_a_mega_kernel<Trait>,
          w_map,
          x_map,
